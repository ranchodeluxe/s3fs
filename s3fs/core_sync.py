...
import boto3
import botocore
from botocore.exceptions import ClientError, HTTPClientError, ParamValidationError, ConnectTimeoutError, ReadTimeoutError
...
class FSSyncTimeoutError(ConnectTimeoutError, ReadTimeoutError):
    """
...
# TODO: review these timeouts
def error_wrapper(func, *, args=(), kwargs=None, retries):
    import time
...
class S3SyncFileSystem(AbstractFileSystem):
    """
    ...
    def get_s3(self, bucket=None):
        # TODO: is this necessary? there is no matching .get_bucket_client
        return self._s3

    def call_s3(self, method, *akwarglist, **kwargs):
        self.set_session()
        s3 = self.get_s3(kwargs.get("Bucket"))
    ...
    def set_session(self, refresh=False, kwargs={}):
        """Establish S3 connection object.
        Returns
        -------
        # TODO: Session to be closed later with await .close()?
        """
        ...
        # TODO: handle sync caching regions and weakref removal
        # logger.debug(
    ...
    @staticmethod
    def close_session(s3):
        # TODO: review if we can force close a session with boto3
    ...
    def find(
        ...
        out = self._lsdir(path, delimiter="", prefix=prefix, **kwargs)
        if not out and key:
            try:
                out = [self.info(path)]
    ...
    def mkdir(self, path, acl=False, create_parents=True, **kwargs):
        ...
        if self.exists(bucket):
            ...
            try:
                ...
                self.call_s3("create_bucket", **params)
    ...
    def rmdir(self, path):
        ...
        try:
            self.call_s3("delete_bucket", Bucket=path)
    ...
    def _lsbuckets(self, refresh=False):
        if "" not in self.dircache or refresh:
            if self.anon:
                ...
                return []
            try:
                files = (self.call_s3("list_buckets"))["Buckets"]
            except ClientError:
    ...
    def ls(self, path, detail=False, refresh=False, versions=False):
    ...
    def exists(self, path):
        ...
        if key:
            ...
            try:
                self.info(path, bucket, key, version_id=version_id)
                return True
            except FileNotFoundError:
                return False
            ...
            try:
                ...
                pass
            try:
                self.call_s3("get_bucket_location", Bucket=bucket, **self.req_kw)
                return True
            except Exception:
                return False
    ...
    def cat_file(self, path, version_id=None, start=None, end=None):
        ...
        if start is not None or end is not None:
            head = {"Range": self._process_limits(path, start, end)}
        else:
            head = {}
        ...
        return error_wrapper(_call_and_read, retries=self.retries)
        ...
        **kwargs,
        ...
        with open(lpath, "rb") as f0:
            if size < min(5 * 2**30, 2 * chunksize):
                ...
                out = self._upload_file_part_concurrent(
                    bucket,
                    key,
                    mpu,
                    f0,
    ...
    def _upload_file_part_concurrent(
        self,
        bucket,
        key,
        mpu,
        ...
        def _upload_chunk(chunk, part_number):
            result = self.call_s3(
                "upload_part",
                Bucket=bucket,
        ...
        while True:
            ...
            if len(chunks) > 1:
                ...
                out.append(_upload_chunk(chunk, len(out) + 1))
        return out
    ...
    def info(self, path, bucket=None, key=None, refresh=False, version_id=None):
        path = self._strip_protocol(path)
    ...
    def chmod(self, path, acl, recursive=False, **kwargs):
        ...
        bucket, key, version_id = self.split_path(path)
        if recursive:
            allfiles = self.find(path, withdirs=False)
            [self.chmod(p, acl, recursive=False) for p in allfiles]
        elif key:
    ...
    def url(self, path, expires=3600, client_method="get_object", **kwargs):
    ...
    def _copy_etag_preserved(self, path1, path2, size, total_parts, **kwargs):
        ...
        part_infos = [
            self.call_s3("head_object", Bucket=bucket1, Key=key1, PartNumber=i)
    ...
    def cp_file(self, path1, path2, preserve_etag=None, **kwargs):
        """Copy file between locations on S3.
    ...
    def list_multipart_uploads(self, bucket):
        out = self.call_s3("list_multipart_uploads", Bucket=bucket)
        return out.get("Contents", []) or out.get("Uploads", [])

    def clear_multipart_uploads(self, bucket):
        """Remove any partial uploads in the bucket"""
        out = self.list_multipart_uploads(bucket)
        [
            self.call_s3(
    ...
    def rm_file(self, path, **kwargs):
        ...
        try:
            self.call_s3("delete_object", Bucket=bucket, Key=key)
        except ClientError as e:
            raise translate_boto_error(e)
    ...
    def rm(self, path, recursive=False, **kwargs):
        if recursive and isinstance(path, str):
            ...
            if not key and self.is_bucket_versioned(bucket):
                ...
                self._rm_versioned_bucket_contents(bucket)
    ...
    def is_bucket_versioned(self, bucket):
        return (self.call_s3("get_bucket_versioning", Bucket=bucket)).get(
            "Status", ""
        ) == "Enabled"

    def make_bucket_versioned(self, bucket, versioned: bool = True):
    ...
    def _rm_versioned_bucket_contents(self, bucket):
        """Remove a versioned bucket and all contents"""
        self.set_session()
        s3 = self.get_s3(bucket)
        pag = s3.get_paginator("list_object_versions")
        for plist in pag.paginate(Bucket=bucket):
            ...
            if obs:
                self.call_s3("delete_objects", Bucket=bucket, Delete=delete_keys)
    ...
    def walk(self, path, maxdepth=None, **kwargs):
        ...
        # TODO: review on fsspec fork
        for _ in super().walk(path, maxdepth=maxdepth, **kwargs):
            yield _
...
class S3SyncFile(AbstractBufferedFile):
    ...
    Examples
    --------
    >>> s3 = S3SyncFileSystem()  # doctest: +SKIP
    >>> with s3.open('my-bucket/my-file.txt', mode='rb') as f:  # doctest: +SKIP
    ...     ...  # doctest: +SKIP
...
def _fetch_range(fs, bucket, key, version_id, start, end, req_kw=None):
    ...
    return error_wrapper(_call_and_read, retries=fs.retries)
